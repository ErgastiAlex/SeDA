...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        # torchrun provides LOCAL_RANK; default to 0 if absent (safety)
    ...
    # IMPORTANT: bind this process to the right GPU before init_process_group
    torch.cuda.set_device(args.gpu)

    # Make sure the rest of the code uses the per-process device
    args.device = f"cuda:{args.gpu}"

    args.dist_backend = 'nccl'
    print('| distributed init (rank {}): {}'.format(args.rank, args.dist_url), flush=True)

    # If your torch supports it, this helps NCCL know the mapping and avoid warnings.
    try:
        ...
        # Older PyTorch: no device_id argument
        torch.distributed.init_process_group(
            backend=args.dist_backend,
            init_method=args.dist_url,
            world_size=args.world_size,
