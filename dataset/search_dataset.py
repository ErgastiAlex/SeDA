...
class search_test_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.be_pose_img:
            ...
            # pose_ann = "test/" + self.ann[index]['image'].split('/')[-1]
            # pose_path = os.path.join(self.image_root, 'pose/' + pose_ann)
            pose = Image.open(pose_path).convert('RGB')
            pose = self.transform(pose)
        else:
            pose = {}
...
class search_train_dataset_decouple(Dataset):
    def __init__(self, config, transform):
        self.image_root = config['image_root']
        self.transform = transform
        self.max_words = config['max_words']
        ...
        for ann in self.ann:
            ...
            if 'normal' in ann:
                self.category_ids[img_id] = 0
            else:
                self.category_ids[img_id] = 1
    ...
    def __getitem__(self, index):
        ...
        dec_caps = ann["decoupled_caption"][0]
        
        if dec_caps[0]["appearance"] == "":
            dec_caps[0]["appearance"] = cap
        if dec_caps[0]["action"] == "":
            dec_caps[0]["action"] = cap
        if dec_caps[0]["background"] == "":
            dec_caps[0]["background"] = cap
            
        if dec_caps[1]["appearance"] == "":
            dec_caps[1]["appearance"] = ann['hard_c']
        if dec_caps[1]["action"] == "":
            dec_caps[1]["action"] = ann['hard_c']
        if dec_caps[1]["background"] == "":
            dec_caps[1]["background"] = ann['hard_c']
        
        dec_cap_appearance = pre_caption(dec_caps[0]["appearance"], self.max_words)
        dec_cap_action = pre_caption(dec_caps[0]["action"], self.max_words)
        dec_cap_bg = pre_caption(dec_caps[0]["background"], self.max_words)
        
        dec_cap_h_appearance = pre_caption(dec_caps[1]["appearance"], self.max_words)
        dec_cap_h_action = pre_caption(dec_caps[1]["action"], self.max_words)
        dec_cap_h_bg = pre_caption(dec_caps[1]["background"], self.max_words)


        return image, caption, caption_eda, self.img_ids[img_id], \
                    pose, hard_i, hard_i_pose, hard_caption, \
                    (dec_cap_appearance, dec_cap_action, dec_cap_bg), \
                    (dec_cap_h_appearance, dec_cap_h_action, dec_cap_h_bg), self.category_ids[img_id]
