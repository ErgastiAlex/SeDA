...
class Search(CMP):
    ...
    def forward(self, image, text_ids, text_atts, text_ids_masked=None, masked_pos=None, masked_ids=None,
                ...
                dec_cap_input=None, cat_id=None
                ):
        ...
        image_feat, image_app_feat, image_act_feat, image_bg_feat  = self.get_image_feat_decoupled(image_embeds)
        text_feat, text_app_feat, text_act_feat, text_bg_feat = self.get_text_feat_decoupled(text_embeds)
        ...
        dec_cap_embeds = self.get_text_embeds(dec_cap_input['input_ids'], dec_cap_input['attention_mask'])
        
        dec_cap_embeds_app, dec_cap_embeds_act, dec_cap_embeds_bg = torch.chunk(dec_cap_embeds, 3, dim=0)
        dec_cap_att_app, dec_cap_att_act, dec_cap_att_bg = torch.chunk(dec_cap_input['attention_mask'], 3, dim=0)
        

        _, dec_cap_feat_app, _, _ = self.get_text_feat_decoupled(dec_cap_embeds_app)
        _, _, dec_cap_feat_act, _= self.get_text_feat_decoupled(dec_cap_embeds_act)
        _, _, _, dec_cap_feat_bg = self.get_text_feat_decoupled(dec_cap_embeds_bg)


        loss_itc_app = self.get_contrastive_loss(image_app_feat, dec_cap_feat_app, idx=idx)
        loss_itc_act = self.get_contrastive_loss(image_act_feat, dec_cap_feat_act, idx=idx)
        loss_itc_bg = self.get_contrastive_loss(image_bg_feat, dec_cap_feat_bg, idx=idx)

        loss_itc_app_txt = self.get_contrastive_loss(text_app_feat, dec_cap_feat_app, idx=idx)
        loss_itc_act_txt = self.get_contrastive_loss(text_act_feat, dec_cap_feat_act, idx=idx)
        loss_itc_bg_txt = self.get_contrastive_loss(text_bg_feat, dec_cap_feat_bg, idx=idx)

        loss_itc = loss_itc + (loss_itc_app + loss_itc_act + loss_itc_bg + loss_itc_app_txt + loss_itc_act_txt + loss_itc_bg_txt) #/ 6.0


        loss_itm_act = self.get_matching_loss(image_embeds, image_atts, image_act_feat,
                                        dec_cap_embeds_act, dec_cap_att_act, text_act_feat, idx=idx)
        loss_itm_app = self.get_matching_loss(image_embeds, image_atts, image_app_feat,
                                        dec_cap_embeds_app, dec_cap_att_app, text_app_feat, idx=idx)
        loss_itm_bg = self.get_matching_loss(image_embeds, image_atts, image_bg_feat,
                                        dec_cap_embeds_bg, dec_cap_att_bg, text_bg_feat, idx=idx)
        
        loss_itm = loss_itm + loss_itm_act + loss_itm_app + loss_itm_bg
