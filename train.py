...
def train_model(model, data_loader, optimizer, scaler, tokenizer, epoch, device, scheduler, config, mask_generator):
    ...
    for i, (image, text, text_eda, idx, pose, hard_i, hard_i_pose, hard_caption, dec_cap, dec_cap_h, cat_id) in enumerate(
        ...
        dec_cap_app, dec_cap_act, dec_cap_bg = dec_cap
        
        # Tokenize each caption component
        dec_cap_app_input = tokenizer(dec_cap_app, padding='max_length', truncation=True,
                                     max_length=config['max_tokens'], return_tensors="pt").to(device)     
        dec_cap_act_input = tokenizer(dec_cap_act, padding='max_length', truncation=True,
                                     max_length=config['max_tokens'], return_tensors="pt").to(device)
        dec_cap_bg_input = tokenizer(dec_cap_bg, padding='max_length', truncation=True,
                                     max_length=config['max_tokens'], return_tensors="pt").to(device)

        # Concatenate input_ids along sequence dimension
        dec_cap_concat_ids = torch.cat([
            dec_cap_app_input.input_ids,
            dec_cap_act_input.input_ids,
            dec_cap_bg_input.input_ids
        ], dim=0)  # [batch_size, 3 * max_tokens]
        
        dec_cap_concat_atts = torch.cat([
            dec_cap_app_input.attention_mask,
            dec_cap_act_input.attention_mask, 
            dec_cap_bg_input.attention_mask
        ], dim=0)

        # Package concatenated inputs
        dec_cap_input = {'input_ids': dec_cap_concat_ids, 'attention_mask': dec_cap_concat_atts}
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            loss_itc, loss_itm, loss_mlm = \
                model(image, text_input.input_ids, text_input.attention_mask,
                      ...
                      dec_cap_input=dec_cap_input, cat_id=cat_id
                      )
